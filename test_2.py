...
def find_model(instance, config):
    ...
    def estimate_min_couriers():
        total_size = sum(sizes)
        max_capacity = max(max_loads)
        return int(np.ceil(total_size / max_capacity))

    min_couriers = estimate_min_couriers()

    for n_couriers in range(min_couriers, n_couriers + 1):
